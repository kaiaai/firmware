...
LDS::result_t LDS_LDROBOT_LD19::start() {
  enableMotor(true);
  postInfo(INFO_MODEL, getModelName());
  return RESULT_OK;
}

uint32_t LDS_LDROBOT_LD19::getSerialBaudRate() {
  return 230400;
}

float LDS_LDROBOT_LD19::getTargetScanFreqHz() {
  return target_scan_freq;
}

int LDS_LDROBOT_LD19::getSamplingRateHz() {
  return 5000;
}

float LDS_LDROBOT_LD19::getCurrentScanFreqHz() {
  static constexpr float ONE_OVER_360 = 1.0f/360.0f;
  return ONE_OVER_360 * speed_deg_per_sec;
}

LDS::result_t LDS_LDROBOT_LD19::stop() {
  enableMotor(false);
  return RESULT_OK;
}

void LDS_LDROBOT_LD19::enableMotor(bool enable) {
...
bool LDS_LDROBOT_LD19::isActive() {
  return motor_enabled;
}

LDS::result_t LDS_LDROBOT_LD19::setScanTargetFreqHz(float freq) {
...
inline uint8_t LDS_LDROBOT_LD19::checkSum(uint8_t value, uint8_t crc) {
...
LDS::result_t LDS_LDROBOT_LD19::processByte(uint8_t c) {
...
uint16_t LDS_LDROBOT_LD19::decodeUInt16(const uint16_t value) const {
  union {
    uint16_t i;
    char c[2];
  } bint = {0x0201};
...
const char* LDS_LDROBOT_LD19::getModelName() { return "LDROBOT LD19"; }